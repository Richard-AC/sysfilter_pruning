...
fn main() {
    ...
    for path in env::args().skip(1) {
        ...
        //let function_type = find_function_type(&dwarf, "main");
        //println!("{:#?}", function_type);
        find_function_pointers_types(&dwarf, "main");
...
-> Result<FunctionType, ()> where <R as Reader>::Offset: LowerHex {
    ...
    while let Some(child) = children.next().unwrap() {
        if child.entry().tag() == gimli::constants::DW_TAG_formal_parameter {
            println!("<{}><{:x}> {}", 1 + 1, child.entry().offset().0, child.entry().tag());
